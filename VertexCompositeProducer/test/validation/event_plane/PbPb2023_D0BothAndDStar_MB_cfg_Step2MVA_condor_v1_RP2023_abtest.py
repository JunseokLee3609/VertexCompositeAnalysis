...
# optional 3rd arg: output subdir under DstarAnalysis
subDir = sys.argv[py_idx+3] if len(sys.argv) > py_idx+3 else 'HIPhysicsRawPrime12'
out_base_dir = '/eos/cms/store/group/phys_heavyions/junseok/DstarAnalysis'
outDir = f"{out_base_dir}/{str(subDir).strip('/')}" if subDir else out_base_dir
try:
    os.makedirs(outDir, exist_ok=True)
except Exception as e:
    raise RuntimeError(f"Failed to create output directory: {outDir} ({e})")
outfile = f"file:{outDir}/d0ana_tree_stepMVA_{outputSuffix}.root"
...
process.GlobalTag.toGet.extend([
    cms.PSet(
        ...
        tag = cms.string("HeavyIonRPRcd_PbPb2023_EventPlaneCalibration_data_offline_v1"),
        connect = cms.string("frontier://FrontierProd/CMS_CONDITIONS")
    )
])
