...
_dir = sys.argv[py_idx+3] if len(sys.argv) > py_idx+3 else 'HIPhysicsRawPrime12'
dirName = os.path.basename(_dir).strip('/')
...
# Add PbPb centrality (read from conditions DB instead of local sqlite), guarded by doCentrality if available
# isMC = False
...
process.TFileService = cms.Service("TFileService",
    fileName =
    cms.string(f'file:{outDir}/d0ana_tree_stepMVA_{outputSuffix}.root')
    )
