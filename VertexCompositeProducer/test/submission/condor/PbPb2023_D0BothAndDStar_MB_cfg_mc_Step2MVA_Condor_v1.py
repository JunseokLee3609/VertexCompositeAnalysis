...
out_dir = '/'.join([out_base_dir] + [part for part in out_dir_parts if part])
out_suffix = os.path.basename(outputSuffix).replace('.root','') or 'default'
if not is_remote_out:
    try:
        os.makedirs(out_dir, exist_ok=True)
    except Exception as e:
        raise RuntimeError(f"Failed to create output directory: {out_dir} ({e})")
outfile = f"{out_dir}/d0ana_tree_stepMVA_{out_suffix}.root"
# Input source
...
# Changed to match data file parameters
...
#process.d0ana_mc.debugGenMatching = cms.untracked.bool(True)
...
process.dStarana_mc.eventplaneSrc = cms.untracked.InputTag("hiEvtPlaneFlatRecalc")
process.dStarana_mc.eventplaneSrcRecalc = cms.untracked.InputTag("hiEvtPlaneFlatRecalc")
#process.dStarana_mc.debugGenMatching = cms.bool(True)
