...
class PATCompositeTreeProducer5OnnxDebug : public edm::one::EDAnalyzer<> {
public:
  explicit PATCompositeTreeProducer5OnnxDebug(const edm::ParameterSet&);
  ~PATCompositeTreeProducer5OnnxDebug();
...
private:
  ...
  void initOnnxDebug();
  void fillOnnxDebug(unsigned int index);
  ...
  // ----------member data ---------------------------
    ...
    float d0fit_cos3D[MAXCAN];
    float d0fit_alpha3D[MAXCAN];
    float d0fit_cos2D[MAXCAN];
    float d0fit_alpha2D[MAXCAN];
    ...
    TH1F* hOnnxDebugMVA_;
    TH1F* hOnnxDebugMVADiff_;
    TH2F* hOnnxDebugMVAVsStored_;
    bool isSkimMVA_;
    bool isCentrality_;
    bool isEventPlane_;
...
bool PATCompositeTreeProducer5::matchHadron(const reco::Candidate* _dmeson_, const reco::GenParticle& _gen_, bool isMatchD0) const {
  bool match = false;
  if(isMatchD0){
...
bool PATCompositeTreeProducer5::matchHadron(const reco::Candidate* _dmeson_, const reco::Candidate& _gen_, bool isMatchD0) const {
  bool match = false;
  if(isMatchD0){
    reco::Candidate const* reco_trk1 = _dmeson_->daughter(0);
    reco::Candidate const* reco_trk2 = _dmeson_->daughter(1);
...
bool PATCompositeTreeProducer5::checkSwap(const reco::Candidate* _dmeson_, const reco::GenParticle& _gen_) const {
    return _dmeson_->pdgId() != _gen_.pdgId();
};
bool PATCompositeTreeProducer5::checkSwap(const reco::Candidate* _dmeson_, const reco::Candidate& _gen_) const {
    return _dmeson_->pdgId() != _gen_.pdgId();
};

bool PATCompositeTreeProducer5::matchTrackdR(const reco::Candidate* _recoTrk_, const reco::Candidate* _genTrk_, bool chkchrg) const {
    bool pass= false;
    // _deltaR_
...
reco::GenParticleRef PATCompositeTreeProducer5::findMother(const reco::GenParticleRef& genParRef)
{
...
void PATCompositeTreeProducer5::genDecayLength(const reco::Candidate& gCand, float& gen_decayLength2D_, float& gen_decayLength3D_, float& gen_angle2D_, float& gen_angle3D_){
...
//void PATCompositeTreeProducer5::getAncestorId(const reco::Candidate& gCand, int& gen_ancestorId_, int& gen_ancestorFlavor_ ){
//  gen_ancestorId_ = 0;
