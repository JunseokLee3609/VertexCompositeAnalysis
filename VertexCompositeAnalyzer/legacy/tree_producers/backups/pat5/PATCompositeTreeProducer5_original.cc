...
D0DaughterSummary summarizeD0Daughters(const CandT* d0) {
        ...
        matchDeltaR3[it] = INVALID_VALUE;
            ...
            if(debugGenMatching_) {
              ...
              DEBUG_GEN("Two-layer decay mode for candidate " << it);
	            ...
	            const auto resetRecoGenMatch = [&](unsigned int idx) {
	              matchGEN[idx] = false;
	              isSwap[idx] = false;
	              idmom_reco[idx] = -77;
	              idBAnc_reco[idx] = -77;
              ...
              if (!validReco) {
                ...
                if (allTracksMatched) {
                  ...
                  idBAnc_reco[it] = bAncestor;
	                  ...
	                  // matchGEN is now reserved for radiative D0 chain in two-layer decay
	                  matchGEN[it] = (allTracksMatched && radiativeChain);
	                  LogDebug("GenMatching") << "Candidate " << it
	                                          << " matchGen_validDstarChain=" << validChain
	                                          << " matchGEN_radiative=" << matchGEN[it];

	                  // For single-layer decay: strict D0 with no extra daughters or FSR
	                  if (!twoLayerDecay_) {
	                    bool validD0chain = false;
	                    if (genD0) {
                      int gammaCount = 0;
                      int nonGamma = 0;
                      bool hasK = false;
                      ...
                      for (size_t idau = 0; idau < genD0->numberOfDaughters(); ++idau) {
                        ...
                        if (!dau) continue;
                        const int absId = std::abs(dau->pdgId());
                        if (absId == 22) { ++gammaCount; continue; }
                        ++nonGamma;
                        if (absId == KAON_PDG_ID) hasK = true;
                        if (absId == PION_PDG_ID) hasPi = true;
                      }
	                      validD0chain = (gammaCount == 0 && nonGamma == 2 && hasK && hasPi);
	                    }
	              ...
	              bool hasGenTrackPool = genpars.isValid() && !genTrackPool.empty();
	              if (hasGenTrackPool && d1 && d2) {
	                std::vector<bool> used(genTrackPool.size(), false);

	                auto gd1Match = findBestTrackMatch(*d1, genTrackPool, used, deltaR_);
	                if (gd1Match.index >= 0 && static_cast<size_t>(gd1Match.index) < used.size()) {
	                  used[gd1Match.index] = true;
	                }
	                auto gd2Match = findBestTrackMatch(*d2, genTrackPool, used, deltaR_);

	                const bool bothTracksMatched = gd1Match.particle && gd2Match.particle;
	                const reco::GenParticle* genD0 = nullptr;
	                if (bothTracksMatched) {
	                  const reco::GenParticle* genFromFirst = findAncestor(gd1Match.particle, D0_PDG_ID);
	                  const reco::GenParticle* genFromSecond = findAncestor(gd2Match.particle, D0_PDG_ID);
	                  if (genFromFirst && genFromSecond && genFromFirst == genFromSecond) {
	                    genD0 = genFromFirst;
	                ...
	                const bool hasKPi =
	                    bothTracksMatched &&
	                    ((std::abs(gd1Match.particle->pdgId()) == KAON_PDG_ID &&
	                      std::abs(gd2Match.particle->pdgId()) == PION_PDG_ID) ||
	                     (std::abs(gd1Match.particle->pdgId()) == PION_PDG_ID &&
	                      std::abs(gd2Match.particle->pdgId()) == KAON_PDG_ID));

	                matchGEN[it] = (bothTracksMatched && genD0 && hasKPi);
	                if (genD0) {
	                  const auto d0Summary = summarizeD0Daughters(genD0);
	                ...
	                if (matchGEN[it] && genD0) {
	                  if(debugGenMatching_) {
	                    LogInfo("PATCompositeTreeProducer") << "D0 track-by-track gen matching SUCCESS for candidate " << it;
	                    DEBUG_GEN("D0 track-by-track gen matching SUCCESS for candidate " << it);
	                  }
	                  ...
	                  matchGen_D0charge_[it] = genD0->charge();
	                  matchGen_D0pdgId_[it] = genD0->pdgId();
	                  isSwap[it] = checkSwap(&trk, *genD0);

	                  const auto* genK =
	                      (std::abs(gd1Match.particle->pdgId()) == KAON_PDG_ID) ? gd1Match.particle : gd2Match.particle;
	                  const auto* genPi = (genK == gd1Match.particle) ? gd2Match.particle : gd1Match.particle;

	                  matchGen_D0Dau1_pT_[it] = genK->pt();
	                  matchGen_D0Dau1_eta_[it] = genK->eta();
	                  matchGen_D0Dau1_phi_[it] = genK->phi();
	                  matchGen_D0Dau1_mass_[it] = genK->mass();
	                  matchGen_D0Dau1_y_[it] = genK->rapidity();
	                  matchGen_D0Dau1_charge_[it] = genK->charge();
	                  matchGen_D0Dau1_pdgId_[it] = genK->pdgId();

	                  matchGen_D0Dau2_pT_[it] = genPi->pt();
	                  matchGen_D0Dau2_eta_[it] = genPi->eta();
	                  matchGen_D0Dau2_phi_[it] = genPi->phi();
	                  matchGen_D0Dau2_mass_[it] = genPi->mass();
	                  matchGen_D0Dau2_y_[it] = genPi->rapidity();
	                  matchGen_D0Dau2_charge_[it] = genPi->charge();
	                  matchGen_D0Dau2_pdgId_[it] = genPi->pdgId();

	                  genDecayLength(*genD0, matchGen_D1decayLength2D_[it], matchGen_D1decayLength3D_[it],
	                                 matchGen_D1angle2D_[it], matchGen_D1angle3D_[it]);
	                  getAncestorId(*genD0, matchGen_D1ancestorId_[it], matchGen_D1ancestorFlavor_[it]);

	                  int momPdg = -77;
	                  int bAncestor = -77;
	                  const reco::GenParticle* mother = dynamic_cast<const reco::GenParticle*>(genD0->mother());
	                  if (mother) momPdg = mother->pdgId();
	                  const reco::GenParticle* ancestor = mother;
	                  int depth = 0;
	                  while (ancestor && depth < 50) {
	                    ancestor = ancestor ? dynamic_cast<const reco::GenParticle*>(ancestor->mother()) : nullptr;
	                ...
	                for (unsigned int igen = 0; igen < nGen; ++igen) {
	                  ...
	                  genDecayLength(*theGenP, matchGen_D1decayLength2D_[it], matchGen_D1decayLength3D_[it],
	                                 matchGen_D1angle2D_[it], matchGen_D1angle3D_[it]);
