...
void PATCompositeTreeProducer5::processCandidates(const CCC* v0candidates_,
    ...
    if (doGenMatching_ && genpars.isValid()) {
        ...
        for (const auto& gen : *genpars) {
            ...
            if (absId != PION_PDG_ID && absId != KAON_PDG_ID) {
                continue;
            }
            genTrackPool.push_back(&gen);
        }
    }
    ...
    for(unsigned it = 0; it < v0candidates_->size(); ++it) {
        ...
        if(doGenMatching_ )
        {
          ...
          if( twoLayerDecay_ ){
            ...
            matchGen_DStarpT_[it] = invalidFloat;
            matchGen_DStareta_[it] = invalidFloat;
            matchGen_DStarphi_[it] = invalidFloat;
            matchGen_DStarmass_[it] = invalidFloat;
            matchGen_DStary_[it] = invalidFloat;
            matchGen_DStarcharge_[it] = 0;
            matchGen_DStarpdgId_[it] = 0;

            matchGen_D0pT_[it] = invalidFloat;
            matchGen_D0eta_[it] = invalidFloat;
            matchGen_D0phi_[it] = invalidFloat;
            matchGen_D0mass_[it] = invalidFloat;
            matchGen_D0y_[it] = invalidFloat;
            matchGen_D0charge_[it] = 0;
            matchGen_D0pdgId_[it] = 0;

            matchGen_D0Dau1_pT_[it] = invalidFloat;
            matchGen_D0Dau1_eta_[it] = invalidFloat;
            matchGen_D0Dau1_phi_[it] = invalidFloat;
            matchGen_D0Dau1_mass_[it] = invalidFloat;
            matchGen_D0Dau1_y_[it] = invalidFloat;
            matchGen_D0Dau1_charge_[it] = 0;
            matchGen_D0Dau1_pdgId_[it] = 0;

            matchGen_D0Dau2_pT_[it] = invalidFloat;
            matchGen_D0Dau2_eta_[it] = invalidFloat;
            matchGen_D0Dau2_phi_[it] = invalidFloat;
            matchGen_D0Dau2_mass_[it] = invalidFloat;
            matchGen_D0Dau2_y_[it] = invalidFloat;
            matchGen_D0Dau2_charge_[it] = 0;
            matchGen_D0Dau2_pdgId_[it] = 0;

            matchGen_D1pT_[it] = invalidFloat;
            matchGen_D1eta_[it] = invalidFloat;
            matchGen_D1phi_[it] = invalidFloat;
            matchGen_D1mass_[it] = invalidFloat;
            matchGen_D1y_[it] = invalidFloat;
            matchGen_D1decayLength2D_[it] = invalidFloat;
            matchGen_D1decayLength3D_[it] = invalidFloat;
            matchGen_D1angle2D_[it] = invalidFloat;
            matchGen_D1angle3D_[it] = invalidFloat;
            matchGen_D1ancestorId_[it] = invalidInt;
            matchGen_D1ancestorFlavor_[it] = invalidInt;
            ...
            matchGen_validD0_noFSR_[it] = false;
            ...
            if (hasGenTrackPool) {
              ...
              if (!validReco) {
                ...
                matchDeltaR1[it] = gd1Match.particle ? gd1Match.deltaR : INVALID_VALUE;
                matchDeltaR2[it] = gd2Match.particle ? gd2Match.deltaR : INVALID_VALUE;
                matchDeltaR3[it] = slowMatch.particle ? slowMatch.deltaR : INVALID_VALUE;

                const bool allTracksMatched = gd1Match.particle && gd2Match.particle && slowMatch.particle;
                matchGEN[it] = allTracksMatched;

                if (!gd1Match.particle && !gd2Match.particle && !slowMatch.particle && debugGenMatching_) {
                  LogDebug("PATCompositeTreeProducer") << "No gen matches found for candidate " << it;
                  DEBUG_GEN("No gen matches found for candidate " << it);
                }

                if (!gd1Match.particle || !gd2Match.particle) {
                  if(debugGenMatching_) {
                ...
                if (!slowMatch.particle && debugGenMatching_) {
                ...
                if (allTracksMatched) {
                  ...
                  matchGen_D0Dau1_pT_[it] = gd1Match.particle->pt();
                  matchGen_D0Dau1_eta_[it] = gd1Match.particle->eta();
                  matchGen_D0Dau1_phi_[it] = gd1Match.particle->phi();
                  matchGen_D0Dau1_mass_[it] = gd1Match.particle->mass();
                  matchGen_D0Dau1_y_[it] = gd1Match.particle->rapidity();
                  matchGen_D0Dau1_charge_[it] = gd1Match.particle->charge();
                  matchGen_D0Dau1_pdgId_[it] = gd1Match.particle->pdgId();

                  matchGen_D0Dau2_pT_[it] = gd2Match.particle->pt();
                  matchGen_D0Dau2_eta_[it] = gd2Match.particle->eta();
                  matchGen_D0Dau2_phi_[it] = gd2Match.particle->phi();
                  matchGen_D0Dau2_mass_[it] = gd2Match.particle->mass();
                  matchGen_D0Dau2_y_[it] = gd2Match.particle->rapidity();
                  matchGen_D0Dau2_charge_[it] = gd2Match.particle->charge();
                  matchGen_D0Dau2_pdgId_[it] = gd2Match.particle->pdgId();

                  matchGen_D1pT_[it] = slowMatch.particle->pt();
                  matchGen_D1eta_[it] = slowMatch.particle->eta();
                  matchGen_D1phi_[it] = slowMatch.particle->phi();
                  matchGen_D1mass_[it] = slowMatch.particle->mass();
                  matchGen_D1y_[it] = slowMatch.particle->rapidity();
                  matchGen_D1charge_[it] = slowMatch.particle->charge();
                  ...
                  if (!genDStar && slowMatch.particle) {
                    genDStar = findAncestor(slowMatch.particle, std::abs(PID_));
                  }
                  ...
                  if (genDStarFromD0 && genD0) {
                    ...
                    const bool d0HasEnough = genD0->numberOfDaughters() >= 2;
                    ...
                    if (d0HasEnough) {
                      d0PDGs = matchD0WithFSR(recoKaonCand, recoPionCand, genD0, deltaR_);
                    }
                  ...
                  // D0 FSR validation: check if D0 has exactly 2 daughters (K and pi) with no FSR
                  bool validD0_noFSR = false;
                  if (genD0) {
                    const size_t nDau = genD0->numberOfDaughters();
                    if (nDau == 2) {
                      const auto* dau0 = genD0->daughter(0);
                      const auto* dau1 = genD0->daughter(1);
                      const int id0 = std::abs(dau0->pdgId());
                      const int id1 = std::abs(dau1->pdgId());
                      const bool isKPi = (id0 == KAON_PDG_ID && id1 == PION_PDG_ID) ||
                                         (id0 == PION_PDG_ID && id1 == KAON_PDG_ID);
                      validD0_noFSR = isKPi;
                    }
                  }
                  matchGen_validD0_noFSR_[it] = validD0_noFSR;
  ...
  std::vector<unsigned int> PATCompositeTreeProducer5::findDaughterPermutation(
      ...
      if(static_cast<int>(nonGammaDauIndices.size()) != nDau) return idxs;  // Wrong number of non-gamma daughters
      ...
      if (!threeProngDecay) {
          ...
          do {
              auto Dd1 = particle.daughter(nonGammaDauIndices[permutations.at(0)]);
              auto Dd2 = particle.daughter(nonGammaDauIndices[permutations.at(1)]);
              auto Dd3 = particle.daughter(nonGammaDauIndices[permutations.at(2)]);
              
              if (abs(Dd1->pdgId()) == PID_dau1_ &&
                  abs(Dd2->pdgId()) == PID_dau2_ &&
                  abs(Dd3->pdgId()) == PID_dau3_) {
                  idxs = {nonGammaDauIndices[permutations.at(0)], 
                          nonGammaDauIndices[permutations.at(1)], 
                          nonGammaDauIndices[permutations.at(2)]};
                  break;
              }
          } while (std::next_permutation(permutations.begin(), permutations.end()));
      }
  ...
  bool PATCompositeTreeProducer5::isValidDStarDecayChain(const reco::Candidate* Dd1, const reco::Candidate* Dd2) const {
      ...
      // Count non-gamma daughters in D0
      int nNonGammaD0Dau = 0;
      bool hasKaon = false;
      bool hasPion = false;
      
      for(size_t i = 0; i < D0->numberOfDaughters(); ++i) {
          const auto* dau = D0->daughter(i);
          if(!dau) continue;
          const int absId = std::abs(dau->pdgId());
          if(absId == 22) continue;  // Skip FSR gamma
          nNonGammaD0Dau++;
          if(absId == KAON_PDG_ID) hasKaon = true;
          if(absId == PION_PDG_ID) hasPion = true;
      }
      
      // D0 must have exactly 2 non-gamma daughters: K and π
      if(nNonGammaD0Dau != 2 || !hasKaon || !hasPion) {
          return false;
      }
  ...
  PATCompositeTreeProducer5::fillGEN(const edm::Event& iEvent, const edm::EventSetup& iSetup)
  {
      ...
      for(unsigned it=0; it<genpars->size(); ++it){
          ...
          if(decayInGen_ && nNonGammaDaughters != 2 && !threeProngDecay_) continue;
          if(decayInGen_ && nNonGammaDaughters != 3 && threeProngDecay_) continue;
      ...
      if(twoLayerDecay_){
        for( unsigned int igen=0; igen<nGen; igen++){
          ...
          // GEN-only strict flags
          int d0NonGamma = 0;
          bool d0HasK = false;
          bool d0HasPi = false;
          for (size_t idau = 0; idau < theGenD0->numberOfDaughters(); ++idau) {
            const auto* dau = theGenD0->daughter(idau);
            if (!dau) continue;
            const int absId = std::abs(dau->pdgId());
          ...
          gen_validD0_noFSR_[igen] = (d0NonGamma == 2 && d0HasK && d0HasPi);
          ...
          gen_validDstarChain_[igen] = (dstarNonGamma == 2 && dstarD0Count == 1 && dstarPiCount == 1 && gen_validD0_noFSR_[igen]);
      }
    }
    else{
        for( unsigned int igen=0; igen<nGen; igen++){
  ...
  { 
          ...
          if(doGenMatching_)
          {
              ...
              if(twoLayerDecay_){
                ...
                PATCompositeNtuple->Branch("matchGen_validDstarChain",&matchGen_validDstarChain_, "matchGen_validDstarChain[candSize]/O");
                 PATCompositeNtuple->Branch("matchGen_validD0_noFSR",&matchGen_validD0_noFSR_, "matchGen_validD0_noFSR[candSize]/O");
    ...
    if(doGenNtuple_)
    {
        ...
        if(twoLayerDecay_){
          ...
          PATCompositeNtuple->Branch("gen_validD0_noFSR",&gen_validD0_noFSR_, "gen_validD0_noFSR[candSize_gen]/O");
        }
        else{
          PATCompositeNtuple->Branch("gen_D0ancestorId_",&gen_D0ancestorId_, "gen_D0ancestorId_[candSize_gen]/I");
          PATCompositeNtuple->Branch("gen_D0ancestorFlavor_",&gen_D0ancestorFlavor_, "gen_D0ancestorFlavor_[candSize_gen]/I");
...
std::vector<reco::GenParticleRef> PATCompositeTreeProducer5::processGenMatching(const edm::Handle<reco::GenParticleCollection>& genpars) {
    ...
    for(unsigned int it = 0; it < genpars->size(); ++it) {
        ...
        // Check daughter PDG IDs based on decay mode (allow FSR gamma)
        bool hasDau1 = false;
        bool hasDau2 = false;
        int nNonGammaDaughters = 0;
        for (size_t i = 0; i < trk.numberOfDaughters(); ++i) {
            const int dauId = std::abs(trk.daughter(i)->pdgId());
            if (dauId == 22) continue;  // Skip FSR gamma
            nNonGammaDaughters++;
            if (dauId == std::abs(PID_dau1_)) hasDau1 = true;  // D0(421) for D*, K(321) for D0
            if (dauId == std::abs(PID_dau2_)) hasDau2 = true;  // π(211) for both
        }

        // For non-threeProngDecay: require exactly 2 non-gamma daughters (allow FSR)
        if(decayInGen_ && (!hasDau1 || !hasDau2) && !threeProngDecay_) continue;
        if(decayInGen_ && nNonGammaDaughters != 2 && !threeProngDecay_) continue;
        ...
        // Collect non-gamma daughter indices for permutation
        std::vector<unsigned int> nonGammaDauIndices;
        for(size_t i = 0; i < trk.numberOfDaughters(); ++i) {
            if(std::abs(trk.daughter(i)->pdgId()) != 22) {
                nonGammaDauIndices.push_back(i);
            }
        }
        ...
        if(static_cast<int>(nonGammaDauIndices.size()) != nDau) {
            LogDebug("GenMatching") << "Candidate " << it << " has wrong number of non-gamma daughters";
            continue;
        }
        ...
        if (!threeProngDecay_) {
            do {
                auto Dd1 = trk.daughter( nonGammaDauIndices[permutations.at(0)] );
                auto Dd2 = trk.daughter( nonGammaDauIndices[permutations.at(1)] );
                if (abs(Dd1->pdgId()) == PID_dau1_ && abs(Dd2->pdgId()) == PID_dau2_) {
                  if(twoLayerDecay_){
                    // D* → D0 + π → K + π + π decay chain validation (FSR allowed)
                    if(!isValidDStarDecayChain(Dd1, Dd2)) {
            ...
            do {
                auto Dd1 = trk.daughter( nonGammaDauIndices[permutations.at(0)] );
                auto Dd2 = trk.daughter( nonGammaDauIndices[permutations.at(1)] );
                auto Dd3 = trk.daughter( nonGammaDauIndices[permutations.at(2)] );
